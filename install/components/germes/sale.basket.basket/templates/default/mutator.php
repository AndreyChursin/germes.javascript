...
foreach ($this->basketItems as $row)
{
	...
	// DEV дорабатыываем arResult корзины
	if(!$rowData['IMAGE_URL']){
		$arItem['IMAGE_URL'] = "";
		$itemRes = CIBlockElement::GetByID($rowData['PRODUCT_ID']);
		if($itemOb = $itemRes->GetNextElement()){
			$item = $itemOb->GetFields();
			$porps = $itemOb->GetProperties();
			$item["PROPS"] = $porps;
			if($item['DETAIL_PICTURE'])
			{
				$arItem['IMAGE_URL'] = CFile::GetPath($item['DETAIL_PICTURE']);
			}
			if($item['PREVIEW_PICTURE'])
			{
				$arItem['IMAGE_URL'] = CFile::GetPath($item['PREVIEW_PICTURE']);
			}
			if($item['PROPS']['MORE_PHOTO']['VALUE'])
			{
				$arItem['IMAGE_URL'] = is_array($item['PROPS']['MORE_PHOTO']['VALUE'])?$item['PROPS']['MORE_PHOTO']['VALUE'][0]:$item['PROPS']['MORE_PHOTO']['VALUE'];
				if(is_int($arItem['IMAGE_URL']))
				{
					$arItem['IMAGE_URL'] = CFile::GetPath($arItem['IMAGE_URL']);
				}
			}
			if($item['PROPS']['PICTURES']['VALUE'])
			{
				$arItem['IMAGE_URL'] = is_array($item['PROPS']['PICTURES']['VALUE'])?$item['PROPS']['PICTURES']['VALUE'][0]:$item['PROPS']['PICTURES']['VALUE'];
				if(is_int($arItem['IMAGE_URL']))
				{
					$arItem['IMAGE_URL'] = CFile::GetPath($arItem['IMAGE_URL']);
				}
			...
			if(!$arItem['IMAGE_URL'] && $rowData['PRODUCT_ID'])
			{
				$item = CCatalogSku::GetProductInfo($rowData['PRODUCT_ID']);
				if($item['ID'])
				{
					$itemRes = CIBlockElement::GetByID($item['ID']);
					if($itemOb = $itemRes->GetNextElement()){
						$item = $itemOb->GetFields();
						$porps = $itemOb->GetProperties();
						$item["PROPS"] = $porps;
		...
		$rowData['IMAGE_URL'] = $arItem['IMAGE_URL'];
	}
	if(!$rowData["NAME"])
	{
		$item = CIBlockElement::GetByID($rowData['PRODUCT_ID'])->GetNext();
		$rowData["NAME"] = htmlspecialchars_decode($item['NAME']);
	}
	// DEV добавим свои метки на товар
	if(
		!empty($rowData)
		&& $rowData["PRODUCT_ID"]
	)
	{
		if(\Bitrix\Main\Loader::includeModule('germes.multiprice')){
			$arStores = \Germes\Sale\Multiprice::getActualQuantity($rowData["PRODUCT_ID"]);
			$canBuyinPercent = \Germes\Sale\Multiprice::getItemBuyPossibility($rowData,$arStores);
			$rowData['STORES'] = $arStores['STORES'];
		}else{
			$arQuantity = \CCatalogProduct::GetByID($rowData["PRODUCT_ID"]);
			$itemQuantity = $arQuantity["QUANTITY"]-$arQuantity["QUANTITY_RESERVED"];
			$buyPossibility = $rowData["QUANTITY"]&&$itemQuantity?($rowData["QUANTITY"]*100/$itemQuantity):0;
			$canBuyinPercent = ($buyPossibility>0&&$buyPossibility<100)?100:($itemQuantity*100/$rowData["QUANTITY"]);
			// определим склады товара
			$res = \Bitrix\Catalog\StoreProductTable::getlist(array(
				'filter' => array("=PRODUCT_ID"=>$rowData["PRODUCT_ID"],'STORE.ACTIVE'=>'Y'),
				'select' => array('AMOUNT',"STORE_NAME"=>'STORE.TITLE', "STORE_DESCR"=>'STORE.DESCRIPTION', "STORE_ADDR"=>'STORE.ADDRESS', 'STORES_' => 'STORE.*'),
				'cache' => array('ttl' => 3600)
			));
			while ($Store = $res->Fetch()){
				$Store["STORE_ID"] = $Store["STORES_ID"];
				$rowData['STORES'][] = $Store;
				/* if($rowData['canBuyinPercent']<100 && preg_match('#_РЦ#i',$Store['TITLE'])){
					$rowData['WHAIT_FROM_RC'][] = $Store;
				} */
			}
		}
		$rowData['CAN_BUY_IN_PERCENT'] = $canBuyinPercent>=0?$canBuyinPercent:0;
		$ANALOGS = FitProject::checkRealAnalogs($rowData);
		$rowData['ANALOGS'] = $ANALOGS;
	}
...
// DEV исправляем итого
if(\Bitrix\Main\Loader::includeModule("germes.multibasket") && \Bitrix\Main\Loader::includeModule("germes.multiprice"))
{
	$arUser = Germes\MultiBasket\MultiBasket::getUserArr();
	$arUser["USER"] = Germes\Sale\MultiPrice::getUserProperty();
	if($totalData['CURRENCY'] != $arUser["USER"]['ACTIVE_CONTRAGENT']['UF_TIPTSEN']["CURRENCY_NAME"])
	{
		$totalData = array(
			'DISABLE_CHECKOUT' => (int)$result['ORDERABLE_BASKET_ITEMS_COUNT'] === 0,
			'PRICE' => $result['allSum'],
			'PRICE_FORMATED' => CCurrencyLang::CurrencyFormat($result['allSum'], $arUser["USER"]['ACTIVE_CONTRAGENT']['UF_TIPTSEN']["CURRENCY_NAME"], true),
			'PRICE_WITHOUT_DISCOUNT_FORMATED' => "",//CCurrencyLang::CurrencyFormat($result['allSum'], $arUser["USER"]['ACTIVE_CONTRAGENT']['UF_TIPTSEN']["CURRENCY_NAME"], true),
			'CURRENCY' => $arUser["USER"]['ACTIVE_CONTRAGENT']['UF_TIPTSEN']["CURRENCY_NAME"]
		);
	}
	$result['DISCOUNT_PRICE_ALL'] = 0;
}
