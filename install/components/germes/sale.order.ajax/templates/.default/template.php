...
else
{
	...
	$arParams["hide_physische_person_block"] = (in_array("7", $user_group) || in_array("9", $user_group))?"Y":"N";
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
				...
				<div class="col-sm-9 bx-soa">
					...
					<!--	REGION BLOCK	-->
					<div id="bx-soa-region" data-visited="false" class="bx-soa-section bx-active<?=$arParams["hide_physische_person_block"]?' d-none':''?>">
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
					...
					<? if ($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'): ?>
						<!--	PAY SYSTEMS BLOCK	-->
						<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active<?=$arParams["hide_physische_person_block"]?' d-none':''?>">
							<div class="bx-soa-section-title-container">
								<h2 class="bx-soa-section-title col-sm-9">
						...
						<!--	DELIVERY BLOCK	-->
						<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active<?=$arParams["hide_physische_person_block"]?' d-none':''?>" <?=($hideDelivery ? 'style="display:none"' : '')?>>
							<div class="bx-soa-section-title-container">
								<h2 class="bx-soa-section-title col-sm-9">
						...
						<!--	PICKUP BLOCK	-->
						<div id="bx-soa-pickup" data-visited="false" class="bx-soa-section<?=$arParams["hide_physische_person_block"]?' d-none':''?>" style="display:none">
							<div class="bx-soa-section-title-container">
								<h2 class="bx-soa-section-title col-sm-9">
	...
	if($arParams[hide_physische_person_block]=="Y"){// скроем лишние типы
		/* foreach ($arResult['JS_DATA']['USER_PROFILES'] as $i => $profile) {
			if($profile['PERSON_TYPE_ID'] == "1")
				unset($arResult['JS_DATA']['USER_PROFILES'][$i]);
		} */
		foreach ($arResult['JS_DATA']['PERSON_TYPE'] as $i => $person_type) {
			if($person_type['ID'] == "1")
				unset($arResult['JS_DATA']['PERSON_TYPE'][$i]);
		}
